...
def gillespie_seir(N, beta_func, delta, gamma, mu, initial_infected, initial_exposed, max_time):
    ...
    return times[:ind+1], susceptible[:ind+1], exposed[:ind+1], infected[:ind+1], recovered[:ind+1], cases[:ind+1]
...
Cum_Cs = np.zeros((num_sims,len(days)))
for i in range(100):
    res = sim()
    ...
    Cum_Cs[i,:] = interp1d(res[0],res[-1],kind='previous',fill_value='extrapolate')(days)
